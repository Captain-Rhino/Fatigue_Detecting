...
class MainUI(QtWidgets.QWidget, main_ui.Ui_Form):
    ...
    def __init__(self):
        ...
        self.sound_thread1 = None
        self.sound_thread2 = None
        self.sound_thread3 = None
            ...
            if (len(faces) != 0):
                ...
                for k, d in enumerate(faces):
                    ...
                    if self.fun[1]:
                        ...
                        if mar > self.MAR_THRESH:  # 张嘴阈值0.5
                            ...
                            if self.mCOUNTER >= self.MOUTH_AR_CONSEC_FRAMES:  # 阈值：3
                                ...
                                self.thread_signal.emit({'type':'msg','value':time.strftime('%Y-%m-%d %H:%M:%S ', time.localtime()) + u"打哈欠"})
                            # 重置嘴帧计数器
                            self.mCOUNTER = 0
                            res[4] = '闭嘴'
                    ...
                    if self.fun[2]:
                        ...
                        roll = euler_angle[2, 0] #roll旋转角度
                        yaw = euler_angle[1, 0]#yaw角
                        #print(roll)
                        #print(yaw)
                        #print(har)
                        if abs(roll) > 10:  # 点头阈值10
                            res[3] = '斜'
                        else:
                            # 如果连续3次都小于阈值，则表示瞌睡点头一次
                          #  if self.hCOUNTER >= self.NOD_AR_CONSEC_FRAMES:  # 阈值：3
                              #  self.hTOTAL += 1
                                #self.thread_signal.emit({'type': 'msg', 'value': time.strftime('%Y-%m-%d %H:%M:%S ',
                                #                                                                   time.localtime()) + u"瞌睡点头"})
                            # 重置点头帧计数器
                            #self.hCOUNTER = 0
                            res[3] = '正'#正斜翻转2023.6.5
                ...
                if self.mTOTAL >= 4:
                    res[1] = '瞌睡'
                elif self.mTOTAL >= self.values[4]:
                    if self.TOTAL >= self.values[0]:
                        res[1] = '瞌睡'
                    else:
                        res[1] = '轻度疲劳'
                ...
                if 'cell phone' in label:
                    res[1] = '使用手机'
                    pass
                if 'cup' in label or 'bottle' in label:
                    res[1] = '喝水'
                    pass
                if 'cell phone' in label and 'cup' in label:
                    res[1] = "危险驾驶"
    ...
    def play_sound1(self):
        if self.sound_thread1 is not None and self.sound_thread1.is_alive():
            # self.plainTextEdit_tip('播放声音中')
            pass
        else:
            self.plainTextEdit_tip.appendPlainText('危险驾驶 播放声音')
            self.sound_thread1 = threading.Thread(target=self.thread_sound1,daemon=True)
            self.sound_thread1.start()
        pass

    def thread_sound2(self):
        pygame.mixer.init()
    ...
    def play_sound2(self):
        if self.sound_thread2 is not None and self.sound_thread2.is_alive():
            # self.plainTextEdit_tip('播放声音中')
            pass
        else:
            self.plainTextEdit_tip.appendPlainText('检测到使用手机，警告提醒')
            self.sound_thread2 = threading.Thread(target=self.thread_sound2,daemon=True)
            self.sound_thread2.start()
        pass

    def thread_sound3(self):
        pygame.mixer.init()
    ...
    def play_sound3(self):
        if self.sound_thread3 is not None and self.sound_thread3.is_alive():
            # self.plainTextEdit_tip('播放声音中')
            pass
        else:
            self.plainTextEdit_tip.appendPlainText('检测到使用手机，警告提醒')
            self.sound_thread3 = threading.Thread(target=self.thread_sound3,daemon=True)
            self.sound_thread3.start()
        pass
